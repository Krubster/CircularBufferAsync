...
public partial class Pipe : IDisposable
{
    ...
    private bool IsLinux { get; set; }
    private bool IsDarwin { get; set; }
    ...
    public Pipe(uint size)
    {
       ...
       // if (Utility.PlatformUtils.IsWindows)
        {
            ...
            {
                throw new InvalidOperationException($"Allocating virtual memory failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }
            ...
            if (!freed)
            {
                throw new InvalidOperationException($"Creating virtual placeholder failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }
            ...
            {
                throw new InvalidOperationException($"Creating file mapping failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }
            ...
            {
                throw new InvalidOperationException($"Mapping file view failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }
            ...
            {
                throw new InvalidOperationException($"Mapping file view mirror failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }
        }/*
        else if (Utility.PlatformUtils.IsLinux || Utility.PlatformUtils.IsDarwin)
        {
            var anon = Utility.PlatformUtils.IsLinux ? NativeMethods_Linux.MAP_ANONYMOUS : NativeMethods_Linux.MAP_ANON;

            int fd;

            if (Utility.PlatformUtils.IsLinux)
            {
            ...
            {
                throw new InvalidOperationException($"Creating file descriptor failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }

            // Set the size of the file descriptor
            if (NativeMethods_Linux.ftruncate(fd, (int)adjustedSize) != 0)
            {
                throw new InvalidOperationException($"Setting file descriptor size failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }
            ...
            {
                throw new InsufficientMemoryException($"Allocating virtual memory failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }
            ...
            {
                throw new InvalidOperationException($"Mapping memory failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }
            ...
            {
                throw new InvalidOperationException($"Mapping mirrored memory failed. ({System.Runtime.InteropServices.Marshal.GetLastPInvokeError()})");
            }
...
#pragma warning restore CA2020
                ...
                _handle = (IntPtr)NativeMethods_Linux.InvalidFileDescriptor;
            }
